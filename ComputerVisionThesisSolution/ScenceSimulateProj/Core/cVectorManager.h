...
#define __CVECTORMANAGER_H__
	...
	protected:
		...
		bool _VectorFound(SString Name,VECTORNODE **pNode=nullptr) const;
		bool _VectorFound(uint32 ID,VECTORNODE **pNode=nullptr) const;
	...
	public:
		...
		unsigned int iRegisterVector(SString Name,uint32 Type=VT_USER,SHANDLE Param=0,int MaxSize=-1,void(*pRelease)(SHANDLE)=nullptr,bool bCreateIndex=false);
		unsigned int iRegisterVector(uint32 ID,uint32 Type=VT_USER,SHANDLE Param=0,int MaxSize=-1,void(*pRelease)(SHANDLE)=nullptr,bool bCreateIndex=false);//////////////////////
		...
		bool iInsertItem(SHANDLE Value,SString strName=_SSTR(""));
		bool iAddItem(SHANDLE Value,SString strName=_SSTR(""));
		bool iSetCurrentItem(SHANDLE Value,SString strName=_SSTR(""));
		bool iDeleteItem();
		bool iMovToFirst();
		bool iMovToLast();
		bool iMovToPerivous();
	...
	inline unsigned int gloRegisterVector(SString Name,uint32 Type=VT_USER,SHANDLE Param=0,int MaxSize=-1,void(*pRelease)(SHANDLE)=nullptr,bool bCreateIndex=false)
