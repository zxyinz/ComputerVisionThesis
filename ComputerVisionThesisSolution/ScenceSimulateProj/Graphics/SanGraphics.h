...
#define __STDSANGRAPHICS_H__
		...
		public:
			...
			stSANCOLORF(const SANCOLOR &color)
				:r(color.r/255.0),g(color.g/255.0),b(color.b/255.0),a(color.a/255.0)
			{};
			~stSANCOLORF(){};
		...
		public:
			...
			union
			{
				lpSANPIXMAP2D pPixMap2D;
				lpSANPIXMAPF2D pPixMap2DF;
				SHANDLE PixMapPtr;
			};
			uint32	DataSize;
			SVALUE	CurrentParam;
		...
		public:
			...
			uint32 ShaderIDArray[SST_SHADER_COUNT];
		...
		typedef list<stSANUNKNOWNEX<SANTEXRENDERDESC, SHANDLE>> SANMATERIALTEXTURELIST;
		...
		struct stSANMATERIAL
		{
			...
			SANCOLORF Emissive;
			sfloat Shininess;
			//list<SHANDLE> TexturePtrList;
			SANMATERIALTEXTURELIST TexList;
