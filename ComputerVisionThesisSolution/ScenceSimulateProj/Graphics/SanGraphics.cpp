...
stSANGRAPHICSPOINT::stSANGRAPHICSPOINT(const stSANGRAPHICSPOINT& P)
{
	...
	if(P.vt_size!=0)
	{
		...
		::gloMemCopy(p_vt,P.p_vt,sizeof(SVECTOR3)*vt_size);
	}
}
stSANGRAPHICSPOINT::~stSANGRAPHICSPOINT()
{
...
stSANGRAPHICSPOINT& stSANGRAPHICSPOINT::operator=(const stSANGRAPHICSPOINT& P)
{
	...
	if(P.vt_size!=0)
	{
		...
		::gloMemCopy(buffer,P.p_vt,vt_size*sizeof(SVECTOR3));
	}
	if(this->vt_size!=0)
	{
...
stSANSKELETONPOINTDESC& stSANSKELETONPOINTDESC::operator=(const stSANSKELETONPOINTDESC& P)
{
	...
	if(P.list_size!=0)
	{
		...
		::gloMemCopy(p_skeletonbuffer,P.skeleton_list_ptr,P.list_size*sizeof(uint32));
		p_weightbuffer=new sfloat[P.list_size];
		::gloMemCopy(p_weightbuffer,P.weight_list_ptr,P.list_size*sizeof(sfloat));
	}
	if(this->list_size!=0)
	{
		delete[] this->skeleton_list_ptr;
...
void stSANXFRAMEELEMENT::SetAnimation(unsigned int index,sfloat Time,sfloat Duration)
{
...
void stSANXFRAMEELEMENT::SetNextAnimation(sfloat Time,sfloat Duration)
{
