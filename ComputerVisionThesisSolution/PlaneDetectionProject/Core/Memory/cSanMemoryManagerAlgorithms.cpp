...
void _ssm_print_mem_state()
{
	...
	for (seek = 0; seek<SSM_MEM_LIST_SIZE; seek = seek + 1)
	{
		...
		if (ssm_page_list[seek].b_is_meg_page == s_true)
		{
			...
			memory_usage = (float) ssm_page_list[seek].allocated_mem / (const float) (SSM_MEM_PAGE_SIZE << SSM_MEG_PAGE_WEIGHT);
		}
		else
		{
			printf("\tis meg page:\t\t\tfalse\n");
			memory_usage = (float) ssm_page_list[seek].allocated_mem / (const float) SSM_MEM_PAGE_SIZE;
		}
