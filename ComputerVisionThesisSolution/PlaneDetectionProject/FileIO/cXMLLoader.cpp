	...
	for (uint32 seek = 0; seek<FileSize; seek = seek + 1)
	{
		...
		case L'<':
			...
			case L'/':
				...
				if (NodeStack.empty())
				{
					this->_PushError(_SSTR("Error: XML file format miss match. Node:") + ::gloWToT(NodeStack.top()));
					delete[] pBuffer;
					pBuffer = nullptr;
					this->iRelease();
					return false;
...
bool cXMLLoader::_CreateNode(swchar* pBuffer, uint32 BufferSize, stSANTREEEX<XMLELEMENT>* pNode, stack<SStringW>* pStack)
{
	if (pBuffer == nullptr)
	{
		return false;
	...
	bool bMarkCount = false;
	for (uint32 seek = 0; seek < BufferSize; seek = seek + 1)
	{
		if (pBuffer[seek] == L'"')
		{
			bMarkCount = !bMarkCount;
		}
		else
		{
			if (bMarkCount)
	...
	vector<SStringW> ItemList = San::gloGetStringItemsW(pBuffer, L" ");
	uint32 StringSize = 0;
	for (uint32 seek = 0; seek < ItemList.size(); seek = seek + 1)
	{
		StringSize = ItemList[seek].size();
		for (uint32 seek_char = 0; seek_char < StringSize; seek_char = seek_char + 1)
		{
			if (ItemList[seek][seek_char] == L'<')
			{
				ItemList[seek][seek_char] = L' ';
			}
		}
	}
	...
	vector<SStringW>::iterator pItem = ItemList.begin();
...
bool cXMLLoader::iSetToPreviousNode()
{
	...
	/*vector<stSANTREEEX<XMLELEMENT>*>::iterator pItem = this->m_pCurrentNode->p_trunk->p_offshootlist.begin();
	for (uint32 seek = 0; seek < (this->m_pCurrentNode->identify - 1); seek = seek + 1)
	{
		pItem++;
	}*/
	this->m_pCurrentNode = this->m_pCurrentNode->p_trunk->p_offshootlist[this->m_pCurrentNode->identify - 1];
	this->m_pCurrentAttribute = this->m_pCurrentNode->data.Attributes.begin();
	return true;
}
...
bool cXMLLoader::iSetToNode(SString strNodeName)
{
	...
	{
		if ((sint) (*pItem)->data.Name.find(::gloTToW(strNodeName)) == 0)
		{
			break;
		}
		pItem++;
...
SString cXMLLoader::iGetCurrentNodeName()
{
	...
	return ::gloWToT(this->m_pCurrentNode->data.Name);
}
SString cXMLLoader::iGetCurrentNodeValue(SString strDefault)
{
	...
	return ::gloWToT(this->m_pCurrentNode->data.Value);
}
...
SString cXMLLoader::iGetFileCodeType()
{
	...
	return ::gloWToT(this->m_pRoot->data.Value);
}