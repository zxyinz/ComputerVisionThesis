...
void PlaneDetectionFunc(_sstream<uint8> &RenderBuffer, const uint32 RenderBufferWidth, const uint32 RenderBufferHeight, _sstream<uint8> &DebugBuffer, const uint32 DebugBufferWidth, const uint32 DebugBufferHeight)
{
	...
	list<_spair<SVECTOR3, SVECTOR3>> NormalPointDataSet = GenerateNormalDataPointsV1(DataStream, RenderBufferSize, SubBuffer, SubBufferSize);
...
list<_spair<SVECTOR3, SVECTOR3>> GenerateNormalDataPointsV2(_sstream<SVECTOR3> &DataStream, const SVECTOR3 &StreamSize, _sstream<sfloat> &SubBuffer, const SVECTOR3 &SubBufferSize)
{
	...
	for (int32 seek_row = 0; seek_row < RowSize; seek_row = seek_row + 1)
	{
		for (int32 seek_col = 0; seek_col < ColSize; seek_col = seek_col + 1)
		{
			//Calculate local normal
			SVECTOR3 Normal(0.0, 0.0, 0.0);
			SVECTOR3 CurrentVec(DataMatrix[seek_row][seek_col]);

			list<SVECTOR3> OffsetVecList;

			int32 StartCol = (seek_col - SearchRadius) < 0 ? 0 : (seek_col - SearchRadius);
			int32 EndCol = (seek_col + SearchRadius) > ColSize ? ColSize : (seek_col + SearchRadius);

			int32 StartRow = (seek_row - SearchRadius) < 0 ? 0 : (seek_row - SearchRadius);
			int32 EndRow = (seek_row + SearchRadius) > RowSize ? RowSize : (seek_row + SearchRadius);

			for (int32 seek_search_row = StartRow; seek_search_row < EndRow; seek_search_row = seek_search_row + 1)
			{
				SVECTOR3* pRowPtr = DataMatrix[seek_search_row];

				for (int32 seek_search_col = StartCol; seek_search_col < EndCol; seek_search_col = seek_search_col + 1)
				{
					SVECTOR3 OffsetVec = pRowPtr[seek_search_col] - CurrentVec;

					sfloat Distance = ::gloLength(OffsetVec);

					if (Distance < Threshold){ continue; }

					OffsetVecList.push_back(::gloNormalize(OffsetVec));
				}
			}
